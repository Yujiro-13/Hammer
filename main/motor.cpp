...
Motor::Motor(gpio_num_t ph_pin_R, gpio_num_t en_pin_R, gpio_num_t ph_pin_L, gpio_num_t en_pin_L, gpio_num_t fan_pin)
{
    ...
    //printf("io_conf.intr_type: %d\n", io_conf.intr_type); // OK
    //printf("io_conf.mode: %d\n", io_conf.mode);          // OK
    //printf("io_conf.pin_bit_mask: %llx\n", io_conf.pin_bit_mask); // OK
    //printf("io_conf.pull_up_en: %d\n", io_conf.pull_up_en); // OK
    //printf("io_conf.pull_down_en: %d\n", io_conf.pull_down_en); // OK
    

    ESP_ERROR_CHECK(gpio_config(&io_conf));
    //printf("GPIO40 set\n");
    ESP_ERROR_CHECK(gpio_set_level(GPIO_NUM_40, 1));

    ESP_ERROR_CHECK(gpio_set_level(BDC_L_MCPWM_GPIO_PH, 0));
    //printf("GPIO41 set\n");
    ESP_ERROR_CHECK(gpio_set_level(BDC_R_MCPWM_GPIO_PH, 0));
    ESP_ERROR_CHECK(gpio_set_level(BDC_L_MCPWM_GPIO_EN, 0));
    ESP_ERROR_CHECK(gpio_set_level(BDC_R_MCPWM_GPIO_EN, 0));
...
void Motor::setMotorSpeed(float spdR, float spdL, float fan)
{
    ...
    if (spdR > 0)
        gpio_set_level(BDC_R_MCPWM_GPIO_PH, 1); //  右モータ反転させてつけた
    else
